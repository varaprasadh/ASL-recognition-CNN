...
#         maskClose, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)

#     if(len(conts) != 0):  # draws the contours of that object which has the highest
#         b = max(conts, key=cv2.contourArea)
#         # gives the co-ordinate of the left extreme of contour
...
#         cv2.circle(frame, (int(centre_x), int(centre_y)), 6,
#                    (255, 0, 0), -1)  # plots centre of the area
...
#             cv2.circle(frame, (int(centre_x), int(centre_y)), 10,
#                        (255, 255, 255), -1)  # plots centre of the area
